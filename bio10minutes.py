...
path = '../bio-data/notiception/3 Ser 2mkM.txt'
# path_neuron = '../../neuron-data/all2.txt'
# path_spike_times = '../../neuron-data/st2.txt'
path_neuron = '../../neuron-data/5HT_res.hdf5'
...
for f in range(1, len(floats) - 1):
	if floats[f - 1] < floats[f] > floats[f + 1] and floats[f] > 0.2:
		spikes_bio.append(f)
...
# plt.plot(intervals, color='blue', marker='.', markersize=10, markerfacecolor='red')
# plt.show()
...
names = ['interspikes intervals', 'number of spikes']
data = [intervals_bio, count_array_bio]
# pyplot.boxplot(data)
